...
import { isResourceAlreadyExistsError } from '@/shared/errors/resource'
...
export type RegisterUserFixture = {
  ...
  whenUserRegisters: (registerUserCommand: RegisterUserCommand) => Promise<void>
...
export const createRegisterUserFixture = (): RegisterUserFixture => {
  ...
  const registerUserUseCase = new RegisterUserUseCase(inMemoryRepository, idProvider, dateProvider)
  ...
  const whenUserRegisters = async (registerUserCommand: RegisterUserCommand): Promise<void> => {
  ...
  const thenRegisteredUsersShouldBe = async (expectedRegisteredUsers: User[]): Promise<void> => {
  ...
  const thenErrorShouldBe = (expectedError?: ResourceAlreadyExistsError): void => {
